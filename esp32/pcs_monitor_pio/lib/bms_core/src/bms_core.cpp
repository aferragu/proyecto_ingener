...
void bms_decode(BmsData& bms, uint8_t bms_addr,
    ...
    if (can_id == (uint32_t)(0x4210 + bms_addr)) {
        uint16_t v_raw = ((uint16_t)d[1] << 8) | d[0];
        uint16_t i_raw = ((uint16_t)d[3] << 8) | d[2];
        uint16_t t_raw = ((uint16_t)d[5] << 8) | d[4];
    ...
    } else if (can_id == (uint32_t)(0x4220 + bms_addr)) {
        uint16_t cv_raw = ((uint16_t)d[1] << 8) | d[0];
        uint16_t dv_raw = ((uint16_t)d[3] << 8) | d[2];
        uint16_t ci_raw = ((uint16_t)d[5] << 8) | d[4];
        uint16_t di_raw = ((uint16_t)d[7] << 8) | d[6];
    ...
    } else if (can_id == (uint32_t)(0x4250 + bms_addr)) {
        bms.status           = d[0] & 0x07;
        bms.force_charge_req = (d[0] >> 3) & 0x01;
        bms.fault            = d[1];
        bms.alarm            = ((uint16_t)d[3] << 8) | d[2];
        bms.protection       = ((uint16_t)d[5] << 8) | d[4];

    } else if (can_id == (uint32_t)(0x4280 + bms_addr)) {

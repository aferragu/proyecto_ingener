...
void test_421_voltage(void) {
    ...
    uint8_t d[] = { 0x60, 0x13,  0x30, 0x75,  0xE8, 0x03,  80, 98 };
    bms_decode(bms, ADDR, 0x4210 + ADDR, d);
...
void test_421_current_discharging(void) {
    ...
    uint8_t d[] = { 0x60, 0x13,  0x00, 0x7D,  0xE8, 0x03,  80, 98 };
    bms_decode(bms, ADDR, 0x4210 + ADDR, d);
...
void test_421_current_charging(void) {
    ...
    uint8_t d[] = { 0x60, 0x13,  0x60, 0x6D,  0xE8, 0x03,  80, 98 };
    bms_decode(bms, ADDR, 0x4210 + ADDR, d);
    TEST_ASSERT_FLOAT_WITHIN(0.5f, -200.0f, bms.current_a);
}
...
void test_421_temperature_25c(void) {
    ...
    uint8_t d[] = { 0x00, 0x00,  0x30, 0x75,  0xF0, 0x04,  80, 98 };
    bms_decode(bms, ADDR, 0x4210 + ADDR, d);
...
void test_422_charge_cutoff_voltage(void) {
    ...
    uint8_t d[] = { 0x50, 0x14,  0xE8, 0x10,  0x00, 0x7D,  0x00, 0x7D };
    bms_decode(bms, ADDR, 0x4220 + ADDR, d);
...
void test_425_status_discharging(void) {
    ...
    bms_decode(bms, ADDR, 0x4250 + ADDR, d);
...
void test_425_alarm_word(void) {
    uint8_t d[] = { 0x00, 0x00, 0x04, 0x01, 0x00, 0x00, 0x00, 0x00 };
    bms_decode(bms, ADDR, 0x4250 + ADDR, d);
    TEST_ASSERT_EQUAL_HEX16(0x0104, bms.alarm);
}

void test_425_protection_word(void) {
    uint8_t d[] = { 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00 };
    bms_decode(bms, ADDR, 0x4250 + ADDR, d);
...
void test_428_charge_forbidden(void) {
    ...
    bms_decode(bms, ADDR, 0x4280 + ADDR, d);
...
void test_wrong_addr_ignored(void) {
    ...
    bms_decode(bms, ADDR, 0x4210 + 2, d);  // addr=2, not our addr=1
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 500.0f, bms.voltage_v);
}
